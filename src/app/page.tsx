...
export default function Home() {
  ...
  const loadDebate = useCallback(async (id: string) => {
    ...
    setThinking(null);
    try {
      const res = await fetch(`/debate?id=${encodeURIComponent(id)}`);
      const data = await res.json();
  ...
  }, [topicInput, refreshDebates, debateActive]);
  ...
  }, [debateId, messages.length, ended]);
        ...
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          {debateActive ? (
            <button
              ...
              onClick={endDebate}
              disabled={endBusy}
              className="hidden shrink-0 rounded-xl border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-semibold text-orange-900 transition hover:bg-orange-100 disabled:opacity-50 dark:border-orange-800 dark:bg-orange-950/40 dark:text-orange-100 dark:hover:bg-orange-950/70 md:inline-flex"
            >
